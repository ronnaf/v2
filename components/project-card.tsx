...
export const ProjectCard = ({ project }: Props) => {
  ...
  return (
    <VStack key={project.id} bg="clay.500" p={6} borderRadius="sm" shadow="md">
      ...
      <VStack alignSelf="flex-start" alignItems="flex-start">
        <Heading as="h1" size="md" fontWeight="700" whiteSpace="nowrap">
          {project.title}
        </Heading>
      ...
      <VStack spacing={1} alignSelf="flex-start" alignItems="flex-start">
        <Flex alignSelf="flex-start" flexWrap="wrap">
          {project.tags.map((tag) => (
            <Text key={tag} as="span" fontSize="xs" color="subtitle.500" whiteSpace="nowrap" _notLast={{ marginRight: 2 }}>
              #{tag}
            </Text>
          ))}
        </Flex>
        {project.builtAt && (
          <Text fontSize={10} color="subtitle.600">
